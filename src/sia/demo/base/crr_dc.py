        ...
        for item in dcTopo.node: 
            if dcTopo.node[item].keys()[0] == 'RACK':
                ...
                for i in range(0, len(neighborList)):
                    if neighborList[i] < item:
                        ...
                        if dcTopo.node[neighborList[i]]['AGGSWITCH']\
                            ...
                            subSet.extend(dcTopo\
                                .node[neighborList[i]]['AGGSWITCH'].cutSet)
                ...
                if tmpHappen == 1:
                    ...
                    self.finalMinimalCut.append(dcTopo.node[item]['RACK'])
            ...
            if self.faultTree.node[node].keys()[0] == 'DATACENTER':
                ...
                for count in range(0, 1000):
                    ...
                    if isHappen == 1:
                        counter += 1.0
                        printResultList\
                                .append(list(set(self.finalMinimalCut)))
                
                printResult = counter/1000.0
                #print('------------ result: %lf'%(printResult))

                rankingList = []
                tmpFailure = 1.0
                for i in range(0, len(printResultList)):
                    for j in range(0, len(printResultList[i])):
                        tmpFailure *= printResultList[i][j].failure
...
def get_failure_of_datacenter(self, node):
        ...
        tmpResult = 1.0
        for i in range(0, len(resultList)):
            tmpResult *= (1 - resultList[def get_failure_of_datacenter(self, node):
        self.minimal_cut = []
        self.finalMinimalCut = []
        numbercounter = 0
        dcTopo = self.faultTree.node[node]['DATACENTER'].topology
