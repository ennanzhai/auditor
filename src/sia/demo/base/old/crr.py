...
class CRR:
        ...
        for appItem in self.appList:
            if appItem.appType == "DATACENTER":
                ...
                if algorithm == "MC":
                    ...
                    self.get_failure_of_app_via_naive(self.faultTree)
                else:
                    pass
            else:
                pass
    ...
    def get_failure_of_app_via_mcs(self):
        ...
        appResult = []
        for i in range(0, len(baseList)):
            if baseList[i] not in appResult:
                appResult.append(baseList[i])
