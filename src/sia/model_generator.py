...
class ModelGenerator:
    def __init__(self, xmlFile):
        ...
        pieceVec = root.getElementsByTagName('chunk')
        for item in pieceVec:
            idTmpList = item.getElementsByTagName('ID')
            tmpID = int(idTmpList[0].childNodes[0].nodeValue)
            ...
            depList = item.getElementsByTagName('dep')
            for i in range(0, len(depList)):
                tmpDep = int(depList[i].childNodes[0].nodeValue)
                self.topology.add_edge(tmpID, tmpDep)
        
        jobVec = root.getElementsByTagName('job')
        for item in jobVec:
            idTmpList = item.getElementsByTagName('ID')
            tmpID = int(idTmpList[0].childNodes[0].nodeValue)
            self.topology.add_node(tmpID)
